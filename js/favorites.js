...
function renderFavorites() {
  ...
  favorites.forEach((product) => {
    ...
    card.className = "border-1 border-gray-300 p-3 rounded-lg shadow-sm bg-white relative";

    card.innerHTML = `
      <img src="${product.image_url}" class="w-full h-48 object-cover rounded">
      <h3 class="mt-2 font-medium text-sm text-gray-600">${product.description}</h3>
      <p class="text-slate-800 font-semibold text-sm mt-1">${product.price_uzs.toLocaleString()} so'm</p>
